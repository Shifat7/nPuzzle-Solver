...
public class AstarStrategy extends SearchMethod {
    ...
    public boolean addToFrontier(PuzzleState aState) {
        ...
        for (PuzzleState p : Searched) {
            //foreach Searched element, if the aState is not in the Searched List,
            //then continue to the next checks otherwise do not
            if (p.equals(aState)) {
    ...
    public direction[] Solve(nPuzzle aPuzzle) {
        ...
        while (Frontier.size() > 0) {
            ...
            //if not the goal state, explore this node
            ...
            for (int i = 0; i < newStates.size(); i++) {
                ...
                if (addToFrontier(newChild)) {
                    //then, work out it's evaluation function by adding its own heuristic value with the cost
                    newChild.HeuristicValue = HeuristicValue(newStates.get(i), aPuzzle.GoalState) + newChild.Cost;
                    newChild.setEvaluationFunction(newChild.HeuristicValue);
                }
            }

            //Sorting the fringe by it's Heuristic Value. The PuzzleComparator uses each nodes EvaluationFunction
