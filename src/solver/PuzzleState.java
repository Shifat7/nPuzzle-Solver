...
public class PuzzleState implements Comparable<PuzzleState>
{
	...
	//the method for printing out the states, uncomment for posible debugging
//	public void getTableForEachState(int[][] aPuzzle){
//		for(int i = 0; i < aPuzzle.length; i++){
//			System.out.print("|");
//			for(int j = 0; j < aPuzzle[0].length; j++){
//				System.out.print(" " + aPuzzle[i][j] + " ");
//
//			}
//			System.out.println("|");
//		}
...
//	}
	...
	private int countMovements(int[] blankLocation)
	{
		...
		try
		{
			...
			for(int i = 0; i <= 1; i++)
			{
				if(i == 0){
					if( i == 0) {
						if(blankLocation[i] == 0 || blankLocation[0] == (Puzzle.length - 1)) {
							//do nothing
						} else {
							result++;
						}
						...
						if(blankLocation[i] == 0 || blankLocation[1] == Puzzle[0].length - 1){
	...
	public int[][] cloneArray(int[][] cloneMe)
	{
