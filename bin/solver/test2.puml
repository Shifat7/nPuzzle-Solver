...
class nPuzzler {
    ...
    public static void main(String[] args) {
        ...
        if (thisMethod == null) {
            //No, give an error
            System.out.println("Search method identified by " + method + " not implemented. Methods are case sensitive. Did you mean BFS, DFS, GBFS or ASTAR?");
            System.exit(1);
        }
...
//			int puzzleSize = Integer.parseInt(bothDimensions[0]);
            ...
            //getting the size of the Width of the NxM puzzle
            int Size_N = Integer.parseInt(bothDimensions[0]);

            //getting the size of the Height of the NxM puzzle
            int Size_M = Integer.parseInt(bothDimensions[1]);


            //initialized the problem and goal puzzle grids using the N and M sizes from the text file
            int[][] startPuzzleGrid = new int[Size_N][Size_M];
            int[][] goalPuzzleGrid = new int[Size_N][Size_M];

            //fill in the start state
            String startStateString = puzzle.readLine();
            startPuzzleGrid = ParseStateString(startStateString, startPuzzleGrid, Size_N);

            //fill in the end state
            String goalStateString = puzzle.readLine();
            goalPuzzleGrid = ParseStateString(goalStateString, goalPuzzleGrid, Size_N);
